...
class Login:
  ...
  def autoset_info(self, session):
    info_query = session.execute(
      "SELECT pass, groupname, ip, mac, enable, cliente_id FROM {0} WHERE user = :username;".format(
        mysqldb_config.login_table), {'username': self.username}).first()
  ...
  def __repr__(self):
    return "<Login(username='{0}',password='{1}',plan='{2}',ip='{3}',mac='{4}',onu='{5}',enable='{6}',\
            client_id='{7}')>".format(self.username, self.password, self.plan, self.ip, self.mac, self.onu, self.enable,
                                      self.client_id)

  def __str__(self):
    return 'username: {0} password: {1} plan: {2} ip: {3} mac: {4} onu: {5} enable: {6} client_id= {7}'.format(
      self.username, self.password, self.plan, self.ip, self.mac, self.onu, self.enable, self.client_id)
...
class Onu:
  ...
  def autoset_vlan_and_macs(self, tn):
    ...
    while not found:
      ...
      logger.debug(
        'Onu: autoset_vlan_and_macs: while not found: waste value: {0}'.format(waste_value.replace('\r', '')))
  ...
  def autoset_signal_power(self, tn):
    ...
    logger.debug('Onu: autoset_signal_power: tn.write: {0}'.format(
      repr('show optic_module slot {0} link {1} onu {2}'.format(self.board, self.pon, self.onu_number))))
    ...
    for pon in pon_list:
      if not found:
        ...
        while not found and not end_of_list:
          ...
          waste_value = get_next_value(tn, '\t')
          logger.debug(
            'Onu: autoset_info_by_mac: while not found: waste_value: {0}'.format(waste_value.replace('\r', '')))
          if 'gponline#' in waste_value:
            end_of_list = True
  ...
  def __repr__(self):
    return "<Onu(phy_address='{0}',board='{1}',pon='{2}',onu_number='{3}',signal_power='{4}',vlan='{5}'," \
           "associated_mac_list='{6}')>".format(self.phy_address, self.board, self.pon, self.onu_number,
                                                self.signal_power, self.vlan, repr(self.associated_mac_list))

  def __str__(self):
    return 'phy_address: {0} board: {1} pon: {2} onu_number: {3} signal_power: {4} vlan: {5} \
            associated_mac_list: {6}'.format(self.phy_address, self.board, self.pon, self.onu_number, self.signal_power,
                                             self.vlan, self.associated_mac_list)
...
class Client:
  ...
  def autoset_info(self, session):
    info_query = session.execute(
      "SELECT cpfcnpj, nome, endereco, numero, complemento, referencia, bairro, status FROM {0} \
      WHERE id = :client_id;".format(mysqldb_config.clientes_table), {'client_id': self.client_id}).first()
    self.cpf = info_query[0]
    self.name = sanitize_name(info_query[1])
    self.address = '{0}, {1}\n{2} {3}\n{4}'.format(info_query[2], info_query[3], sanitize_dumb(info_query[4]),
                                                   sanitize_dumb(info_query[5]), info_query[6])
    self.enable = True if info_query[7] == 1 else False

  def __repr__(self):
    return "<Client(cpf='{0}',name='{1}',address='{2}',enable='{3}',logins='{4}')>".format(self.cpf, self.name,
                                                                                           self.address, self.enable,
                                                                                           repr(self.logins))

  def __str__(self):
    return 'cpf: {0} name: {1} address: {2} enable: {3} logins: {4}'.format(self.cpf, self.name, self.address,
                                                                            self.enable, self.logins)
...
def find_clients_by_onu(session, onu):
  ...
  for associated_mac in onu.associated_mac_list:
    current_login_query = session.execute(
      "SELECT DISTINCT UserName FROM {0} WHERE CallingStationID = :mac ORDER BY AcctStartTime DESC LIMIT 1;".format(
        mysqldb_config.radius_acct_table), {'mac': associated_mac}).first()
...
def print_clients(clients):
  ...
  for client in clients:
    if client.cpf not in printed_clients:
      logger.info('\n{0}\n{1}\n{2}\n{3}'.format(client.name, client.cpf, client.address,
                                                'Ativo' if client.enable else 'Bloqueado'))
      ...
      if login.username not in printed_logins:
        logger.info(
          '\n{0} {1}\n{2} {3}\n{4} {5}\n{6} P{7} PON{8} ONU{9}{10} CLIENTE {11}\nVLAN: {12} Power: {13}\n{14}'.format(
            login.username, login.password, login.plan, 'Ativo' if login.enable else 'Bloqueado', login.ip, login.mac,
            login.onu.phy_address, login.onu.board, login.onu.pon, '0' if int(login.onu.onu_number) < 10 else '',
            login.onu.onu_number, login.username.upper(), login.onu.vlan, login.onu.signal_power,
            '\n'.join(login.onu.associated_mac_list)))
        printed_logins.append(login.username)


def find_onu_by_user(session, tn, username):
  login_query = session.execute(
    "SELECT CallingStationID FROM {0} WHERE UserName = :username ORDER BY AcctStartTime DESC LIMIT 1;".format(
      mysqldb_config.radius_acct_table), {'username': username}).first()
  if login_query:
    mac = login_query[0]
    onu = Onu(tn, mac=mac)
...
def main():
  ...
  with Telnet(telnet_config.ip, telnet_config.port) as tn:
    connect_su(tn)
    if onu:
      print_clients(find_clients_by_onu(session, Onu(tn, string=onu)))
    elif username:
