...
from tasiap.common.bot_common import is_user_authorized, get_signal
from tasiap.common.mysql_common import supply_mysql_session, user_exists
...
from tasiap.user_from_onu import is_cto_id
    ...
    if not len(context.args):
      ...
      if is_onu_id_valid(context.args[0]):
        ...
        if onu_id['onu_id']:
          ...
          update.message.reply_text('{0}\nTente novamente informando o ID ou serial da ONU.'.format(
            onu_id['diagnostic']), quote=True)
        else:
          update.message.reply_text(
