...
class TestOutputMethods(unittest.TestCase):
  ...
  def test_get_onu_number(self):
    ...
    success = 'show pon_mac slot 12 link 1 lookup E4BEED22E4F3\r\n-----  PON MAC ADDRESS, ITEM=208 ' \
              '-----\n\r203\tE4:BE:ED:22:E4:F3\t Vid:1119\t OnuId:19\n\rAdmin\\gponline# '.encode('ascii').decode(
                                                                                                                'ascii')
    error = 'show pon_mac slot 12 link 1 lookup A1AAAA11A1A1\r\n-----  PON MAC ADDRESS, ITEM=208 ' \
            '-----\n\rAdmin\\gponline# '.encode('ascii').decode('ascii')
